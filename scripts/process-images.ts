...
const SUPPORTED_EXTS = new Set(['.jpg', '.jpeg', '.png', '.tif', '.tiff', '.webp', '.avif', '.heic', '.heif']);
...
function parseNominatimAddress(address: Record<string, string>): string {
  const poi = address.tourism || address.historic || address.amenity || address.building || address.road || address.leisure;
 ...
 */
export function generateOutputStem(meta: ImageMetadata, placeName: string, fallbackStem: string, usedStems: Set<string>): string {
  const d = meta.dateTaken;
  const datePart = d ? `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}` : '';
