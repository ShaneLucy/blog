...
describe("photo detail load()", () => {
  ...
  test("throws 404 for an unknown trip slug", () => {
    expect(() => load({ params: { slug: "does-not-exist", photoSlug: firstPhoto.slug } } as never)).toThrow();
  });

  test("throws 404 for an unknown photo slug", () => {
    expect(() => load({ params: { slug: firstTrip.slug, photoSlug: "does-not-exist" } } as never)).toThrow();
  });
